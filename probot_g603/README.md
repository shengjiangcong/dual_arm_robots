...
This repository provides ROS support for PROBOT G603.   
...
$ git clone https://github.com/ps-micro/PROBOT_G603
```
...
1. Run PROBOT [install.sh](https://github.com/ps-micro/PROBOT_G603/install.sh)
```sh
...
```sh
$ roslaunch probot_bringup probot_g603_bringup.launch sim:=true
```
...
```sh
$ roslaunch probot_g603_moveit_config demo_gazebo.launch
```

### __MoveIt! with real Hardware__  
There are launch files available to bringup a real robot(PROBOT G603).   
...
```sh
$ roslaunch probot_bringup probot_g603_bringup.launch robot_ip:=192.168.2.123
```
